...
Note that you must use the https://prisma.pan.dev/api/cloud/api-urls/[API URL] for your tenant. This example uses https://api.prismacloud.io.
+
[userinput]
----
curl --request POST \
...
----
+
The value for token in the response is the JWT you will use to authorize your subsequent REST API requests.
